...
In normal use, this instance keeps recently active tickets pre-scored in the background. Most agent traffic hits the fast, repeat-lookup path, no matter how many agents are online. The slow case above only shows up when new tickets arrive faster than the background pass can score them.
...
When the archive changes, the system re-scores every open ticket in the background, so agents keep seeing pre-computed results. These numbers are a projection from the measured per-ticket cost, not a live run at that scale.

| Open tickets | Refresh time |
|---|---|
| 50 | ~15s |
| 200 | ~1 min |
| 500 | ~2.5 min |
...
- **Scale tested:** 67 archived tickets, 5 open at once. This is a small validation corpus, not a production-size archive. The refresh-time table above is a projection, not a live run at that scale.
...
- Agents mostly work tickets that have been open a while, so repeat lookups make up most of the traffic.
