...
Points 1-3 (the Postgres cache, the RQ worker, and the `retrieval/indexing.py` choke point) implemented and verified end-to-end (issue #10). Point 4 (the Frappe bridge app + Vue edit) implemented and verified end-to-end (issue #11) against the isolated Docker Helpdesk instance (`ticket-rag-helpdesk-dev-frappe-1`), not the shared local bench that hosts other real client sites -- see `docs/PROPOSED_ARCHITECTURE.md`'s history for why. `frappe_bridge/ticket_match_bridge/`'s override calls `GET /tickets/{ticket_name}/matches` and enriches each Match with `status`/`priority`/`creation` via `frappe.get_all` -- Frappe-owned metadata the retrieval API never needed to carry. One addition beyond the original design: `resolution_details` is HTML, so the override strips tags (`frappe.utils.strip_html_tags`) before handing it to the Vue snippet -- rendering raw `<p>` tags verbatim in a plain-text list item was a real bug caught in browser verification, not a hypothetical.

Two deviations from what's written above. First: `worker` was **not** added as a `docker-compose.yml` service. Locally the API already runs directly via `uvicorn` rather than containerized (the `Dockerfile` is for `docker-compose.prod.yml` only); the worker follows that same existing pattern and runs directly via `python -m worker.run`, not as a new compose service. Only `postgres` and `redis` (infra, like `qdrant`) were added to `docker-compose.yml`. Second: `bench get-app <path>` (this ADR's stated install command) failed against this bench version for a bare local path with no git remote -- installed instead via `pip install -e apps/ticket_match_bridge` plus a manual `sites/apps.txt` entry, which is what `get-app` does under the hood for a local app anyway. Revisit the `docker-compose.yml` deviation when `docker-compose.prod.yml` is built (`docs/DEPLOYMENT_PLAN.md`). This second deviation turned out not to be a bench-version quirk -- see [ADR 0008](0008-standalone-bridge-app-repo.md), which found the real cause and gave the app its own repo.
